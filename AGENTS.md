...
- **세션이 둘이면 maestro 를 동시에 돌리지 않는다.** 기기를 나눠도 소용없다 —
  2026-09-19 에 두 세션이 각자 기기에서 돌렸는데 한쪽 플로우가 계속 죽었고,
  화면에 멀쩡히 보이는 요소가 '못 찾음'이 되거나 엉뚱하게 Safari 가 앞으로 나왔다.
  돌리기 전에 `ps -eo pid,command | grep '[m]aestro --device'` 로 남이 쓰는지 본다.
