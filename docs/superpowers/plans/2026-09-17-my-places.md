...
**매칭 규칙 회귀 테스트도 같이 쓴다.** Step 3이 `pick()`의 방향을 줄이는데, 그걸 지키는 테스트가 없으면 다음 사람이 도로 넓힌다. 같은 파일에 덧붙인다 — `placesStore`가 테스트 스텁에서 빈 목록이라, 이 테스트는 **먼저 `savePlace()`로 '집'을 심고** 돌린다(`plan.test.ts`의 스텁이 `expo-file-system`을 이미 가짜로 끼우므로 저장은 메모리에만 남는다):
...
664-679행. **주소 스프레드(`destinationAddress`·`originAddress`)는 이미 들어가 있다** — Task 1~3 최종 리뷰가 "Task 4~9 동안 틀린 주소가 기기에 쌓인다"고 지적해 그때 선반영했다. 아래 블록은 그 상태를 포함한 최종형이니 그대로 맞추면 된다. 이 과제가 실제로 바꾸는 건 `known`의 출처와 `pick()`의 매칭 규칙 둘이다:
