...
**Interfaces:**
- Consumes: Task 1의 `parsePlaces` / `serializePlaces` / `upsertRecent` / `upsertSaved` / `removeSaved` / `knownPlaces` / `EMPTY_PLACES` / 타입 `PlacesFile`·`SavedPlace`·`RecentPlace`
- Produces:
...
function load(): PlacesFile {
  try {
    const f = file();
    return f.exists ? parsePlaces(f.textSync()) : EMPTY_PLACES;
  } catch {
