...
`src/screens/CalculatingScreen.tsx` — import 두 줄을 더하고:
...
(이미 `usePlan`을 쓰고 있으면 중복 import 하지 않는다. 쓰고 있지 않으면 컴포넌트 안에 `const { state } = usePlan();`을 더한다.)

35-43행의 effect를 이렇게 바꾼다:
...
```ts
  useEffect(() => {
    ...
    // 적재는 시작과 같은 생애에 한 번이면 된다 — state를 의존성에 넣으면 매 렌더 재평가된다
