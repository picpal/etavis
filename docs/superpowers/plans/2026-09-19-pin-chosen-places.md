...
- [x] **2~4.** 실패 확인 → 구현 → 초록
- [ ] **5.** **시뮬레이터 — 막혀 있다(2026-09-19 18:40).** 워커가 `/places` 와
      `/route`(transit) 에 **503** 을 준다. 트랙 로그:
      `plan.fail {kind:"search", message:"Error: places keyword 503"}`.
      **내 변경과 무관한 라이브 장애**이고, 데스크톱 세션이 `main` 에서 그걸 고치는 중이다
      (`kvWrite.ts`, Workers Logs, `8815f62 슬롯 하나의 검색 실패로 계획 전체를 죽이지 않는다`).

      **목 데이터셋으로는 대신 못 잰다.** 목 스톱(`asStopState`)은 `baseId` 가 풀 id 라
      칩 id 와 안 맞고, 이 기능 전체가 그 매칭 위에 서 있다 — 목에서는 고정도 칩 라벨도
      아예 켜지지 않는다. 서버가 돌아온 뒤에 봐야 한다.

      볼 것: 재현 절차(여의도→용왕산, 대중교통, `마트 들러서 장 보고 약국 갔다 집에 갈게`
      확정 → `빵집도 들러줘`)에서 **약국이 그대로인지**. 트랙 로그의 `plan.slots` 에서
      그 슬롯의 `calls` 가 **0 이 아닌 것**(검색은 돌아야 한다)과 `fix=<id>(source)`,
      그리고 `plan.result` 의 `picked` 가 고정된 가게를 담는지 함께 본다.
