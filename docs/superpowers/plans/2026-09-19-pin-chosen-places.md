...
- [x] **2~4.** 실패 확인 → 구현 → 초록
- [ ] **5.** **시뮬레이터 — 절반만 봤다(2026-09-19 21:00).**

      | 본 것 | 근거 |
      |---|---|
      | 실제 계산이 된다 | `plan.result picked='온누리평안약국 · 한청할인마트'` |
      | **검색이 돈다**(과제 4의 전제) | `plan.slots` — 약국 20곳 `calls=4`, 마트 12곳 `calls=4` |
      | 확정이 스톱을 싣는다 | `plan.apply {stops: "온누리평안약국 · 한청할인마트"}` |
      | 칩 라벨 fallback | 미확정 상태에서 `마트 ▾` `약국` — 없는 사실을 안 지어낸다 |

      **못 본 것 — 여기가 이 기능의 본체다.** 확정 뒤 `빵집도 들러줘` 를 넣었을 때
      **약국이 그대로인지**, 그리고 **칩이 가게 이름을 말하는지**.

      막은 것은 서버가 아니라 **maestro** 다(9회 시도, 전부 도구 문제).
      함정은 `AGENTS.md` 의 시뮬레이터 절에 적었다 — 특히 **세션 둘이 maestro 를 동시에
      돌리면 기기를 나눠도 깨진다**. 손으로 하면 30초다.

      **목 데이터셋으로는 대신 못 잰다.** 목 스톱(`asStopState`)은 `baseId` 가 풀 id 라
      칩 id 와 안 맞고, 이 기능 전체가 그 매칭 위에 서 있다 — 목에서는 고정도 칩 라벨도
      아예 켜지지 않는다.

      볼 것: 여의도→용왕산, 대중교통, `마트 들러서 장 보고 약국 갔다 집에 갈게` 확정
      → 편집 → 대화로 수정 → `빵집도 들러줘`. 트랙 로그에서는 그 슬롯의 `calls` 가
      **0 이 아닌 것**(검색은 돌아야 한다)과 `fix=<id>(source)`, 그리고 `plan.result` 의
      `picked` 가 고정된 가게를 담는지 본다.
