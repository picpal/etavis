...
test('접두가 안 맞으면 채택하지 않는다 — 우연한 겹침을 막는다', () => {
  // 60m 밖에 두어 '가까운 게 하나뿐' 규칙이 끼어들지 않게 한다(≈70.5m)
...
test('정규화 이름 일치는 접두 일치보다 우선한다 — 규칙 2 격리', () => {
  // 0번 35.3m 접두만 일치('스타벅스'), 1번 79.3m 정확히 같음('스타벅스강남')
  // 규칙 2가 없으면 0번을 반환하므로 이 테스트는 규칙 2을 검증한다
  const i = matchPlace(at('스타벅스강남'), [at('스타벅스', 0.0004), at('스타벅스강남', 0.0009)]);
  assert.equal(i, 1);
});

test('접두 일치는 60m 밖에서도 작동한다 — 규칙 3 격리', () => {
  // 61.7m 밖, 접두만 일치, 후보 하나
  // 규칙 3이 없으면 null을 반환하므로 이 테스트는 규칙 3을 검증한다
  const i = matchPlace(at('스타벅스'), [at('스타벅스강남', 0.0007)]);
  assert.equal(i, 0);
});
